...
import game.unit.{DefaultMonsterFactory, Monster, MonsterFactory}
...
object DefaultQuest {
  ...
  def createQuest(level: Int, loot: Seq[ItemType], monsterName: String, monsterFactory: MonsterFactory): Quest = {
    val monster = monsterFactory.generateMonster(level, monsterName)
    new DefaultQuest(monster, loot, level)
  }

  def createQuest(level: Int, loot: Seq[ItemType], monsterName: String): Quest = {
    createQuest(level, loot, monsterName, new DefaultMonsterFactory())
  }
}
